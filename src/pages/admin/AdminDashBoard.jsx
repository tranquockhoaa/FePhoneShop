  ...
  const CustomRevenueTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          <p style={{ margin: "0 0 4px 0", fontWeight: "bold" }}>{selectedMonth ? `Ngày ${label}` : `${label}`}</p>
          <p style={{ margin: "0", color: "#1890ff" }}>Doanh thu: {data.revenue?.toLocaleString("vi-VN")} ₫</p>
        </div>
      )
    }
    return null
  ...
  const CustomOrdersTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          <p style={{ margin: "0 0 4px 0", fontWeight: "bold" }}>{selectedMonth ? `Ngày ${label}` : `${label}`}</p>
          <p style={{ margin: "0", color: "#52c41a" }}>Đơn hàng: {data.orders?.toLocaleString("vi-VN")}</p>
        </div>
      )
    }
    return null
  ...
  const CustomCombinedTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          <p style={{ margin: "0 0 8px 0", fontWeight: "bold" }}>{selectedMonth ? `Ngày ${label}` : `${label}`}</p>
          <p style={{ margin: "0 0 4px 0", color: "#1890ff" }}>Doanh thu: {data.revenue?.toLocaleString("vi-VN")} ₫</p>
          <p style={{ margin: "0", color: "#52c41a" }}>Đơn hàng: {data.orders?.toLocaleString("vi-VN")}</p>
        </div>
      )
    }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        ...
        >
          ...
          <p style={{ margin: "0 0 4px 0", color: "#8c8c8c", fontSize: "12px" }}>Mã: {data.code}</p>
          <p style={{ margin: "0", color: "#1890ff" }}>Đã bán: {data.totalSold?.toLocaleString("vi-VN")} sản phẩm</p>
        </div>
      )
    }
    return null
      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
        <div style={{ marginBottom: "24px" }}>
          <Row justify="space-between" align="middle" gutter={[16, 16]}>
            <Col>
              ...
              <p style={{ color: "#8c8c8c", margin: "4px 0 0 0" }}>Theo dõi doanh thu và sản phẩm bán chạy</p>
            </Col>
        ...
        <Spin spinning={loading}>
          ...
          <Row gutter={[24, 24]} style={{ marginBottom: "24px" }}>
            <Col xs={24} lg={12}>
              <Card
                title={selectedMonth ? "Doanh Thu Theo Ngày" : "Doanh Thu Theo Tháng"}
                extra={
            ...
            <Col xs={24} lg={12}>
              <Card
                title={selectedMonth ? "Số Đơn Hàng Theo Ngày" : "Số Đơn Hàng Theo Tháng"}
                extra={
                  <span style={{ color: "#8c8c8c", fontSize: "14px" }}>
                    {selectedMonth
          ...
          <Row gutter={[24, 24]}>
            <Col xs={24}>
              <Card
                ...
                extra={
                  <span style={{ color: "#8c8c8c", fontSize: "14px" }}>
                    Biểu đồ cột các sản phẩm bán chạy nhất - Dễ nhìn và trực quan
                  </span>
                }
              >
                <div style={{ height: "450px" }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={bestSellingChartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                      ...
                      />
                      <YAxis tick={{ fontSize: 12 }} tickFormatter={(value) => value.toLocaleString("vi-VN")} />
                      <Tooltip content={<CustomBestSellingTooltip />} />
                      <Bar dataKey="totalSold" radius={[8, 8, 0, 0]} fill="url(#colorGradient)" />
                      <defs>
                        <linearGradient id="colorGradient" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor="#fa8c16" stopOpacity={1} />
                          <stop offset="100%" stopColor="#ffd666" stopOpacity={0.8} />
                        </linearGradient>
                      </defs>
                    </BarChart>
                  </ResponsiveContainer>
